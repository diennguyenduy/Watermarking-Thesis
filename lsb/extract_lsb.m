function varargout = extract_lsb(varargin)
    ...
    watermarked_image = varargin{1};	%Ảnh đã nhúng thủy vân và cần trích xuất là input thứ 1
    ...
    while k < len
        k = k+1;
        for j = 1:8
            index = (k-1)*8 + j;	%Xét 8 pixel một lúc của ảnh chứa thủy vân (Để lấy ra 1 pixel của thủy vân)
            b = bitget(watermarked_array(p(index)),1);  %Lấy ra bit ở vị trí thứ 1 (LSB) của pixel j của ảnh chứa thủy vân
            if(b == 1)                                  %Nếu bit đó = 1
